...
[JsonObject]
public record SemanticTokensOptions : IWorkDoneProgressOptions {
    /// <inheritdoc />
    public bool WorkDoneProgress { get; set; }

    public SemanticTokensLegend Legend { get; init; } = SemanticTokensLegend.Default;
    ...
    /// </summary>
    public OptionsFull? Full { get; init; }

    /// <summary>
    /// The legend used by the server
    /// </summary>
    public record SemanticTokensLegend(string[] TokenTypes, string[] TokenModifiers) {
        public uint EncodeType(string tokenType) =>
            (uint)Math.Max(0, Array.IndexOf(TokenTypes, tokenType));

        public uint EncodeMods(IList<string>? mods) {
            uint result = 0;
            if (mods != null)
                foreach (var mod in mods)
                    result |= (1u << Math.Max(0, Array.IndexOf(TokenModifiers, mod)));
            return result;
        }

        public static readonly SemanticTokensLegend Default = 
            new(DefaultSemanticTokenTypes.Values, DefaultSemanticTokenModifiers.Values);
    }
